...
type BookingLockerInterface interface {
	AcquireLock(ctx context.Context, technicianID, bayID string, start time.Time) (bool, error)
	ReleaseLock(ctx context.Context, technicianID, bayID string, start time.Time) error
}
...
func bookingLockKey(technicianID, bayID string, start time.Time) string {
	return fmt.Sprintf("lock:booking:tech:%s:bay:%s:slot:%d",
		technicianID, bayID, start.UTC().Unix())
}
...
func (l *BookingLocker) AcquireLock(ctx context.Context, technicianID, bayID string, start time.Time) (bool, error) {
	key := bookingLockKey(technicianID, bayID, start)
...
func (l *BookingLocker) ReleaseLock(ctx context.Context, technicianID, bayID string, start time.Time) error {
	key := bookingLockKey(technicianID, bayID, start)
